...
| L45 | The pen is a pointer-only tool: a stroke is a drag and there is no keyboard path to drawing one. It does not create the gap L14 records, but it widens it | **open**, part of L14 |
| L46 | Hiding fixed overlays from the first screenful is not offered. `HIDE_FIXED_CSS` is indiscriminate, so on a site whose header is `position: fixed` it would produce a capture with no header at all. Telling page chrome from a transient overlay is a real problem and F15's manual picker sidesteps it rather than solving it | **by design**, D66 |
...
| L47 | A page whose scripts never go quiet can leave `executeScript` pending forever. Every step across into a page now has a deadline, so the capture is delivered short rather than lost, but the underlying step is still Chrome's to resolve and there is no way to ask it for a partial answer | **by design**, D27 |
| L48 | A capture finished with Finish now, or ended by a stalled step, is trimmed to the last screenful taken. On a wide page a stall can still leave the final row short down its right-hand side; a deliberate stop cannot, because it only lands at the start of a row | **open**, low priority, D28 |
...
| L41 | A text shape had no width of its own, so editing a caption widened its box instead of wrapping into it | **closed** 2026-09-10, F43, D60. Two side handles set a wrap width, the height follows the words, and a caption saved without one measures exactly as it did |
| L44 | A word wider than the wrap overflows its caption rather than being broken. Breaking inside a word is a thing a screenshot caption never wants, and the alternative to overflowing is a loop that cannot terminate | **by design**, D60 |
| L17 | Inline text entry positions its `<input>` once, from the canvas bounding box, which is correct only while the canvas scrolls with the document | **closed** 2026-09-10, D58. F6 shipped without a scroll container: zoom is a CSS width on the canvas and the page goes on doing the scrolling, so the case this was watching for cannot arise |
| L43 | Zoom is not remembered between captures. Every capture is a different size, so a percentage carried over from the last one means nothing, and a fit is what the old rule did anyway | **by design**, D58 |
