...
| **F45** | Open an image to edit, not only one you captured. Paste, drop, and an Open button left of Download. The result tab opened with no capture is a landing surface now, not a dead end. Raster only by an explicit list, both canvas limits checked, EXIF orientation honoured, and a paste over an open picture goes to a new tab | `src/lib/open.js`, `src/ui/result.js` |
| **F4** | Freehand pen and object eraser. The first shape whose payload is a list, so it is thinned at the source, simplified once on release, and never edited in place. An eraser drag is one undo step. Numbered steps gained the resize handles they never had | `src/lib/geometry.js`, `src/lib/edit.js`, `src/ui/editor.js`, D65 |
